...
from .forms import PendaftaranPosterForm
...
def daftar_poster_view(request):
    form = PendaftaranPosterForm()

    if request.method == 'POST':
        form = PendaftaranPosterForm(request.POST, request.FILES)
        if form.is_valid():
            form.save()
            return redirect(to='daftar-sukses-poster')

    context = {
        'form': form,
    }
    return render(request, 'itech/daftar-poster.html', context)
...
def daftarsuksesposter_view(request):
    context = {}
    return render(request, 'itech/daftar-sukses-poster.html', context)
